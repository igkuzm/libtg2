...
uint32_t deserialize_ui32(buf_t *b){
	...
	b->data += 4;
	b->size -= 4;
	//*b = buf_new_data(b->data + 4, b->size - 4);
	return c;
}
...
uint64_t deserialize_ui64(buf_t *b){
	...
	b->data += 8;
	b->size -= 8;
	//*b = buf_new_data(b->data + 8, b->size - 8);
	return c;
}
...
buf_t deserialize_buf(buf_t *b, int size){
	buf_t ret = buf_new_data(b->data, size);
	b->data += size;
	b->size -= size;
	//*b = buf_new_data(b->data + size, b->size - size);
	return ret;
}
...
buf_t deserialize_bytes(buf_t *b)
{
  ...
  buf_t byte = buf_new_data(b->data, 4);
  int offset = 0;
  uint32_t len = 0;
  ...
  if (byte.data[0] <= 253) {
    len = byte.data[0];
		// skip 1 byte
		//*b = buf_new_data(b->data + 1, b->size - 1);
		b->data += 1;
		b->size -= 1;
	...
	if (pad) {
		//*b = buf_new_data(b->data + pad, b->size - pad);
		b->data += pad;
		b->size -= pad;
	}
