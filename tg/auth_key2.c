 ...
 /* Server sends response of the form
	...
	*/
	if (tl == NULL || tl->_id != id_resPQ){
		ON_ERR(tg, "%s: expected resPQ, but got: %s",
 ...
 /* data_with_padding := data + random_padding_bytes; --
	...
	*     data is not longer than 144 bytes. */
	...
	buf_t random_padding_bytes = 
		buf_new_rand(192 - data.size);
	buf_t data_with_padding = 
		buf_new_bufs(data, random_padding_bytes);
...
generation_new_random_temp_key:;
	...
	buf_t temp_key_data_with_padding = 
		buf_new_bufs(temp_key, data_with_padding);
	...
	buf_t key_aes_encrypted = 
		buf_new_bufs(temp_key_xor, aes_encrypted);
	 if (key_aes_encrypted.size != 256){
		ON_ERR(tg, 
				"%s: key_aes_encrypted len mismatch: expexted 256: has: %d",
